# -*- coding: utf-8 -*-
"""
Created on Fri Oct 20 23:19:27 2017
...
if __name__ == "__main__":
    ...
    parser.add_option('-t', metavar=' ', default='tcp', help='transport type: rtu, tcp, mapped [default: tcp]')
    parser.add_option('-a', metavar=' ', type='int', default=1, help='modbus slave address [default: 1]')
    parser.add_option('-i', metavar=' ', default='localhost',
                      help='ip address to use for modbus tcp [default: localhost]')
    parser.add_option('-P', metavar=' ', type='int', default=502, help='port number for modbus tcp [default: 502]')
    parser.add_option('-p', metavar=' ', default='/dev/ttyUSB0',
                      help='serial port for modbus rtu [default: /dev/ttyUSB0]')
    parser.add_option('-b', metavar=' ', default=9600, help='baud rate for modbus rtu [default: 9600]')
    parser.add_option('-T', metavar=' ', type='float', default=2.0,
    ...
    try:
        if options.t == 'tcp':
            sd = client.SunSpecClientDevice(client.TCP, options.a, ipaddr=options.i, ipport=options.P,
                                            timeout=options.T)
        elif options.t == 'rtu':
            sd = client.SunSpecClientDevice(client.RTU, options.a, name=options.p, baudrate=options.b,
                                            timeout=options.T)
    ...
    if sd is not None:
        ...
        def V_VAr_getter(values):
            ...
            for index, key in enumerate(sd.device.models_list[6].blocks[1].points):
                ...
                if index <= 1:
                    continue
                #nur V
                if index <= 2*ActPt+1 and int(key[-1]) != lstNmbr:
                    v_value = sd.device.models_list[6].blocks[1].points[key].value_getter()
                    lstNmbr += 1
                #nur VAr
                elif index <= 2*ActPt+1 and int(key[-1]) == lstNmbr:
                    var_value = sd.device.models_list[6].blocks[1].points[key].value_getter()
                    pov = (v_value, var_value)
                    values.append(pov)
            print("Liste V- und VAr-Werte: " + str(values))
        ...
        # sucht aus dem Model die eingestellte Anzahl an Werten heraus

        V_VAr_getter(values)

        newV1 = input("Neuer V1-Wert: ")
        sd.volt_var.curve[1].V1 = newV1
        sd.volt_var.write()
        ...
        '''for index, point in enumerate(sd.device.models_list[6].blocks[1].points_list):
            print(str(index)  +  ": " + str(point.value_base))
        print("Blocks in volt_var.model")
        for index, block in enumerate(sd.device.models_list[6].blocks):
            print(block)'''
